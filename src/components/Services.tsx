...
const Services = ({ t }: { t: any }) => {
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      const serviceFrames = gsap.utils.toArray<HTMLElement>('.service-frame');

      gsap.set(serviceFrames, { autoAlpha: 0 });
      gsap.set(serviceFrames[0], { autoAlpha: 1 });
      ...
      // Updated to use the correct variable 'serviceFrames' and typed 'i' as a number
      serviceFrames.forEach((frame: any, i: number) => {
        if (i === 0) return; 
        
        tl.to(serviceFrames[i - 1], { autoAlpha: 0, duration: 1 }) 
